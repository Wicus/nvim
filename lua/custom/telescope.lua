...
local function search_git_status_add_untracked(lines_with_numbers, opts)
...
M.search_status = function(opts)
	...
	vim.fn.jobstart("git diff-index -U0 HEAD", {
		...
		on_exit = vim.schedule_wrap(function(j, output) search_git_status_add_untracked(lines_with_numbers, opts) end),
	})
end
